...
let orchestrator, MockXdefTokenMonetaryPolicy, mockXdefTokenMonetaryPolicy, MockDownstream, mockDownstream
let r
...
async function setupContracts () {
    ...
    MockXdefTokenMonetaryPolicy = await ethers.getContractFactory('MockXdefTokenMonetaryPolicy')
    mockXdefTokenMonetaryPolicy = await MockXdefTokenMonetaryPolicy.deploy()
    await mockXdefTokenMonetaryPolicy.deployed()
    mockXdefTokenMonetaryPolicy = mockXdefTokenMonetaryPolicy.connect(deployer)

    const XdefTokenOrchestrator = await ethers.getContractFactory('XdefTokenOrchestrator')
    orchestrator = await upgrades.deployProxy(XdefTokenOrchestrator, [mockXdefTokenMonetaryPolicy.address])
...
describe('XdefTokenOrchestrator', () => {
    before('setup XdefTokenOrchestrator contract', setupContracts)
    ...
    describe('when rebase called by a contract which is being constructed', () => {
        it('should fail', async () => {
            ...
            // constructorRebaseCallerContract = await upgrades.deployProxy(ConstructorRebaseCallerContract, [mockXdefTokenMonetaryPolicy.address])
    ...
    describe('when transaction list is empty', async () => {
        ...
        it('should call rebase on policy', async () => {
            const fnCalled = MockXdefTokenMonetaryPolicy.interface.decodeEventLog('FunctionCalled', r.events[0].data)
            expect(fnCalled[0]).to.equal('XdefTokenMonetaryPolicy')
